...
class Settings:
    ...
    async def settings(self, ctx):
        ...
        await ctx.send(
            ":information_source: | **Available options: ``prefix`` ; ``resetprefix``**",
            delete_after=15,
        )
    ...
    async def prefix(self, ctx, prefix):
        ...
        if len(prefix) > 50 or len(prefix) < 1:
            await ctx.send(
                ":warning: | **Prefix can't be longer than 50 characters or shorter than 1 character**",
                delete_after=15,
            )
        else:
            async with self.bot.db.acquire() as con:
                a = await con.fetchval(
                    f"SELECT * FROM prefix WHERE GUILDID = {ctx.guild.id};"
                )
                if a is not None:
                    await con.execute(
                        'UPDATE prefix SET prefix=$1 WHERE "guildid"=$2;',
                        prefix,
                        ctx.guild.id,
                    )
                    await ctx.send(
                        f":white_check_mark: | **Updated guild prefix to ``{prefix}``**".replace(
                            "@", "@\u200b"
                        ),
                        delete_after=15,
                    )
                    ...
                    await con.execute(
                        "INSERT INTO prefix (GUILDID, PREFIX) VALUES ($1, $2)",
                        ctx.guild.id,
                        prefix,
                    )
                    await ctx.send(
                        f":white_check_mark: | **Set guild prefix to ``{prefix}``**".replace(
                            "@", "@\u200b"
                        ),
                        delete_after=15,
                    )
