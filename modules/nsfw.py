...
class NSFW:
    ...
    async def _neko(self, ctx, *, tag: str = None):
        ...
        try:
            ...
            embedneko.set_footer(
                text=f"{ctx.author.name}, an error occured. API trouble or no tags found for {tag}.",
                icon_url=ctx.author.avatar_url,
            )
            await ctx.send(embed=embedneko)
                ...
                try:
                    ...
                    async with self.session.get(
                        ...
                        try:
                            ...
                            emberrr.add_field(
                                name="Link to UrbanDictionary:",
                                value=f'**[➤ Definition - {data["list"][0].get("word")}]({data["list"][0].get("permalink")})**',
                                inline=True,
                            )
                            emberrr.set_footer(
                                text="Error while sending message. Maybe, the definition is too long.",
                                icon_url=ctx.author.avatar_url,
                            )
