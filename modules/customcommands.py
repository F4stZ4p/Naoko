...
class CustomCommands:
    ...
    async def customcommand(self, ctx):
        ...
        await ctx.send(
            ":information_source: | **Options available: ``add``, ``remove``, ``list``, ``removeall``**"
        )
    ...
    async def add(self, ctx, commandname: str, *, commandtext: str):
        ...
        async with self.bot.db.acquire() as con:
            if (
                await con.fetchval(
                    "SELECT * FROM customcommands WHERE guildid = $1 AND name = $2",
                    ctx.guild.id,
                    commandname,
                )
            ) is None:
                try:
                    await con.execute(
                        'INSERT INTO customcommands ("guildid", "name", "action") VALUES ($1, $2, $3)',
                        ctx.guild.id,
                        commandname,
                        commandtext,
                    )
                    ...
                    await ctx.send(
                        f":warning: | **Your custom command is too long or error has occured: `{e}`**",
                        delete_after=5,
                    )
            else:
                try:
                    await con.execute(
                        "UPDATE customcommands SET action = $1 WHERE guildid = $2 AND name = $3",
                        commandtext,
                        ctx.guild.id,
                        commandname,
                    )
            ...
            try:
                ...
                self.commands[ctx.guild.id] = {commandname: commandtext}
            await ctx.send(
                f":information_source: | **Custom command `{commandname}` successfully added**",
                delete_after=5,
            )
        await self.bot.db.release(con)
    ...
    async def remove(self, ctx, *, commandname):
        ...
        async with self.bot.db.acquire() as con:
            try:
                await con.execute(
                    "DELETE FROM customcommands WHERE guildid = $1 AND name = $2",
                    ctx.guild.id,
                    commandname,
                )
    ...
    async def removeall(self, ctx):
        ...
        async with self.bot.db.acquire() as con:
            try:
                await con.execute(
                    "DELETE FROM customcommands WHERE guildid = $1", ctx.guild.id
                )
                await ctx.send(
    ...
    async def list(self, ctx):
        ...
        try:
            await NaokoPaginator(
                entries=[c for c in self.commands[ctx.guild.id]],
                length=3,
