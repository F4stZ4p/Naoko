...
class ImageModule:
    ...
    def do_drake_meme(drake_bytes: bytes, no: str, yes: str, path):
        with Image.open(BytesIO(drake_bytes)) as image:
            ...
            draw.text((250, 20), fill(no if len(no) < 50 else no[:50], 15), (0, 0, 0), font=ImageFont.truetype(path, 40))
            draw.text((250, 250), fill(yes if len(yes) < 50 else yes[:50], 15), (0, 0, 0), font=ImageFont.truetype(path, 40))
            bytes_io = BytesIO()
    ...
    async def circle(self, ctx, *, member: discord.Member = None):
        ...
        async with ctx.typing():
            ...
            final_buffer = await self.bot.loop.run_in_executor(None, partial(self.processing, avatar_bytes, member_colour))

            await ctx.send(content=f"<:Ok:501773759011749898> | **Generated for {ctx.author.mention}**", file=discord.File(filename=f"{member.name}.png", fp=final_buffer))

    @commands.command()
    @commands.cooldown(1.0, 10.0, commands.BucketType.user)
    async def filter(self, ctx, color: commands.clean_content, *, member: discord.Member = None):
        """Filter an avatar
        ...
        try:
            async with ctx.typing():
                avatar_bytes = await self.get_avatar(member)
                final_buffer = await self.bot.loop.run_in_executor(None, partial(self.do_filter, avatar_bytes, color))
                await ctx.send(content=f"<:Ok:501773759011749898> | **Generated for {ctx.author.mention}**", file=discord.File(filename=f"{member.name}.png", fp=final_buffer))
        except:
            await ctx.send(f'<:Error:501773759217401856> | **Please, check the color (`{color}` is not correct, for example `#170041` is correct)**', delete_after=15)
    ...
    async def drake(self, ctx, no, *, yes):
        ...
        async with ctx.typing():
            ...
            final_buffer = await self.bot.loop.run_in_executor(None, partial(self.do_drake_meme, drake_bytes, no, yes, f'{self.bot.path}/fonts/Azoft-Sans.ttf'))
            await ctx.send(content=f"<:Ok:501773759011749898> | **Generated for {ctx.author.mention}**", file=discord.File(filename=f"Drake.png", fp=final_buffer))
