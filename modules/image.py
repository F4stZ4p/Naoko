...
class ImageModule:
    ...
    def do_drake_meme(drake_bytes: bytes, no: str, yes: str, path):
        with Image.open(BytesIO(drake_bytes)) as image:
            ...
            draw.text((250, 20), fill(no if len(no) < 50 else no[:50], 17), (0, 0, 0), font=ImageFont.truetype(path, 40))
            draw.text((250, 250), fill(yes if len(yes) < 50 else yes[:50], 17), (0, 0, 0), font=ImageFont.truetype(path, 40))
            bytes_io = BytesIO()
