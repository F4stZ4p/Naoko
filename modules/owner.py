...
class Admin:
        ...
        try:
            ...
            fooem.add_field(
                ...
                value=f"```\n{e.__class__.__name__}: {e}\n```".replace(
                    self.bot.http.token, "•" * len(self.bot.http.token)
                ),
            )
            fooem.set_footer(
    ...
    async def wdiscard(self, ctx):
        self.bot.whitelist = [340_745_895_932_854_272]
        await ctx.send(":ok_hand:")
    ...
    async def restart(self, ctx):
        """Restarts the bot"""
        await ctx.send(
            embed=discord.Embed(color=0x1D99F3).set_footer(
                text="Restarting...", icon_url="https://i.imgur.com/mXZbOEz.png"
            )
        )
    ...
    async def shell(self, ctx):
        await ctx.author.send(
            f"{ctx.prefix}shell hastebin / textfile / default ``<Your Query>``"
        )
    ...
    async def default(self, ctx, *, shellcommand):
        emb = await ctx.send(
            embed=discord.Embed(color=0x67D655).set_footer(
                text="Running...", icon_url="https://i.imgur.com/el2Wofz.gif"
            )
        )
            ...
            try:
                async with self.bot.session.post(
                    ...
                    await emb.edit(
                        ...
                        .add_field(
                            name="**Done. Output too long, uploaded to hastebin**",
                            value=f"**[Click](https://hastebin.com/{post['key']})**",
                        )
                        .set_footer(text="Action done at:", icon_url=ctx.me.avatar_url)
                    )
            except BaseException:
                await emb.edit(
                    ...
                    .add_field(
                        name="**Done. Output too long, uploaded to hastebin**",
                        value=f"**Uploading to hastebin failed. Sorry.**",
                    )
                    .set_footer(text="Action done at:", icon_url=ctx.me.avatar_url)
                )
        del b
    ...
    async def textfile(self, ctx, *, shellcommand):
        ...
        try:
            ...
            await ctx.send(
                embed=discord.Embed(color=0x67D655, timestamp=ctx.message.created_at)
                .add_field(name="**Done**", value="Output redirected to Text File.")
                .set_footer(text="Action done at:", icon_url=ctx.me.avatar_url),
                file=discord.File(b, filename="Shell.txt"),
            )
        except Exception as e:
        ...
        try:
            emb = await ctx.send(
                embed=discord.Embed(color=0x67D655).set_footer(
                    ...
                    icon_url="https://i.imgur.com/el2Wofz.gif",
                )
            )
            await self.bot.db.execute(query)
            await emb.edit(
                embed=discord.Embed(color=0x67D655, timestamp=ctx.message.created_at)
                .add_field(name="**SQL Query Executed Successfully!**", value=chr(173))
                .set_footer(text="Action done at:", icon_url=ctx.me.avatar_url)
            )
        except Exception as e:
            await emb.edit(
                embed=discord.Embed(color=0x67D655, timestamp=ctx.message.created_at)
                .add_field(
                    name="**SQL Query Execution Failed!**",
                    value=f"```css\n[Error: {e}]```",
                )
                .set_footer(text="Action done at:", icon_url=ctx.me.avatar_url)
            )
    ...
    async def reload(self, ctx, *, module):
        try:
            ...
            embed = discord.Embed(color=1_507_393)
            embed.set_footer(
                text="Reloaded {} module.".format(module),
                icon_url="https://raw.githubusercontent.com/F4stZ4p/resources-for-discord-bot/master/cogs.ico",
            )
            await ctx.send(embed=embed, delete_after=10)
        except Exception as e:
    ...
    async def update(self, ctx):
        ...
        emb = await ctx.send(
            embed=discord.Embed(color=0x67D655).set_footer(
                text="Updating...", icon_url="https://i.imgur.com/el2Wofz.gif"
            )
        )
        b = await self.bot.loop.run_in_executor(
            None, _update, os.path.dirname(os.path.realpath(__file__))
        )
        await emb.edit(
            embed=discord.Embed(color=0x67D655, timestamp=ctx.message.created_at)
            .add_field(name="**Updated**", value=f"```fix\n{b}```")
            .set_footer(text="Updated at:", icon_url=ctx.me.avatar_url)
        )
        self.bot.stat = await self.bot.loop.run_in_executor(
