...
class Music:
    ...
    async def _track_hook(self, event):
        ...
        if isinstance(event, lavalink.Events.TrackStartEvent):
            ...
            await channel.send(embed=discord.Embed(title='Now playing:',
                                                   ...
                                                   color=random.randint(0x000000, 0xFFFFFF)).set_thumbnail(url=f'https://img.youtube.com/vi/{event.track.identifier}/hqdefault.jpg').set_footer(icon_url=req.avatar_url, text=f"Track requested by {req.name}"))
        ...
        async with ctx.typing():
            ...
            if isinstance(results, dict):
                ...
                if results['loadType'] == 'PLAYLIST_LOADED':
                    ...
                    if tracks[0]["info"]["length"] > 18000000 and ctx.author.id not in self.bot.patrons:
                        return await ctx.send(embed=discord.Embed(title="Naoko PRO", color=random.randint(0x000000, 0xFFFFFF), description="Can't enqueue songs which are longer than 5 hours\nIf you want to, then donate and get **PRO** - **live** and **unlimited** tracks, **playnow**, **repeat** commands and more!\n\n**[Click me!](https://patreon.com/F4stZ4p)**", timestamp=ctx.message.created_at).set_footer(text=ctx.author.name, icon_url=ctx.author.avatar_url).set_thumbnail(url=self.thumbnail))
                ...
                if 'list' in query and 'ytsearch:' not in query:
                    ...
                    if results[0]["info"]["length"] > 18000000 and ctx.author.id not in self.bot.patrons:
                        return await ctx.send(embed=discord.Embed(title="Naoko PRO", color=random.randint(0x000000, 0xFFFFFF), description="Can't enqueue songs which are longer than 5 hours\nIf you want to, then donate and get **PRO** - **live** and **unlimited** tracks, **playnow**, **repeat** commands and more!\n\n**[Click me!](https://patreon.com/F4stZ4p)**", timestamp=ctx.message.created_at).set_footer(text=ctx.author.name, icon_url=ctx.author.avatar_url).set_thumbnail(url=self.thumbnail))
    ...
    async def _previous(self, ctx):
        ...
        try:
            ...
            await ctx.send(':warning: | **There is no previous song to play**', delete_after=5)
    ...
    async def _queue(self, ctx):
        ...
        if not player.queue:
            return await ctx.send(':information_source: | **The queue is empty now. Go and queue something :smiley:**', delete_after=10)

        await NaokoPaginator(title=f"{ctx.guild.name} Music Queue", length=5, colour=random.randint(0x000000, 0xFFFFFF), entries=[f'`{index + 1}` - [**{track.title}**]({track.uri})' for index, track in enumerate(player.queue)]).paginate(ctx)
    ...
    async def _bassboost(self, ctx, level: str = None):
        ...
        await ctx.send(f':information_source: | **Bass Boost is now `{k}`**', delete_after=5)
    ...
    async def _repeat(self, ctx):
        ...
        await ctx.send(f"🔁 | **Repeat {'enabled' if player.repeat else 'disabled'}**", delete_after=5)
    ...
    async def _remove(self, ctx, index: int):
        ...
        if index > len(player.queue) or index < 1:
            return await ctx.send(f':warning: | **Index has to be between 1 and {len(player.queue)}**', delete_after=15)
