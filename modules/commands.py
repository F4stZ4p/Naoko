...
class Commands():
    ...
    async def upvote(self, ctx):
        ...
        await ctx.send(embed=discord.Embed(color=random.randint(0x000000, 0xFFFFFF), timestamp=ctx.message.created_at, title="Upvote me for a big thanks!", url="https://discordbots.org/bot/444950506234707978/vote").set_image(url="https://discordbots.org/api/widget/444950506234707978.png?topcolor=D896FF&datacolor=823ed6&highlightcolor=D896FF&labelcolor=5d2f77&certifiedcolor=9c4cff").set_footer(text=ctx.author, icon_url=ctx.author.avatar_url))
    ...
    async def about(self, ctx):
        ...
        await ctx.send(embed=discord.Embed(color=random.randint(0x000000, 0xFFFFFF), timestamp=ctx.message.created_at).set_image(url="https://discordbots.org/api/widget/444950506234707978.png?topcolor=D896FF&datacolor=823ed6&highlightcolor=D896FF&labelcolor=5d2f77&certifiedcolor=9c4cff").add_field(name=":bar_chart: Naoko Statistics", value=f'**[Support Guild](https://discord.gg/y3Ph9Nj) | [Patreon](https://www.patreon.com/f4stz4p) | [Invite](https://discordapp.com/oauth2/authorize?client_id=444950506234707978&permissions=8&scope=bot) | [GitHub](https://github.com/NaokoDiscordBot/Naoko) | [Upvote](https://discordbots.org/bot/444950506234707978/vote)**\n\n<:online:452823889177870336> Alive for: **{days}D, {hours}H, {minutes}M, {seconds}S**\n:information_source: Latest update:\n```ini\n{self.bot.stat}```').set_footer(text=f'Bringing fun to Discord since {ptime(ctx.me.created_at)} | {ctx.author}', icon_url=ctx.author.avatar_url))
    ...
    async def source(self, ctx):
    	...
    	"""
    	await ctx.send(embed=discord.Embed(timestamp=ctx.message.created_at, title="Source Code", description=":link: Source code can be found in **title**\n:information_source: Feel free to submit a **Pull Request**", url="https://github.com/NaokoDiscordBot/Naoko", color=random.randint(0x000000, 0xFFFFFF)).set_thumbnail(url="https://i.imgur.com/46eHxTO.png").set_footer(text=ctx.author.name, icon_url=ctx.author.avatar_url))
